...
function EditCanteenProduct({route,navigation}) {
  ...
  function deleteData(){
    Alert.alert(
      ...
      [
        ...
        { text: "OK", onPress: () => {
          setLoad(true);
          axios.post('https://vlu-ewallet.herokuapp.com/library-manager/deleteProduct', {
             id:id
           }).then(res =>{
             if(res.data == true ) {
               setLoad(false);
               navigation.navigate('Thư viện',{post:id});
             }else{
               setLoad(false);
  ...
  function updateProduct(){
    ...
    image:picture,
    barcode:barcode}
    axios.post('https://vlu-ewallet.herokuapp.com/library-manager/editProduct', value).then(res =>{
       if(res.data == 'Success' ) {
         setLoad(false);
         navigation.navigate('Thư viện',{post:id});
       }else{
      ...
      <View style={styles.container}>
          ...
          <View style={{flexDirection:"row", alignItems:'center'}}>
            <View style={styles.inputContainer}>
              ...
              />
              <TouchableOpacity style={styles.barcode} onPress={()=>navigation.navigate('Scan',{screen:'editLibrary'})}>
          ...
          <TouchableOpacity style={styles.icon} onPress={pickImage}>
            ...
            <TouchableOpacity style={styles.icon} onPress={()=>navigation.navigate('Camera',{screen:'editLibrary'})}>
